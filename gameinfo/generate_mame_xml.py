...
try:
    ...
    curs.execute(
        "select c.name, c.description, c.year, c.players, c.ctrltype, c.manufacturer, r.category, m.rotate, m.type, m.ways, m.buttons "
    ...
    for row in curs.fetchall():
        ...
        if row[6]:
            ET.SubElement(game, "category").text = row[6]
        if row[2] and not row[2]=='Swap' and not row[2]=='theme':
            if row[7] and (row[7] == "90" or row[7] == "270"):
                ET.SubElement(game, "orientation").text = "vertical"
            else:
                ET.SubElement(game, "orientation").text = "horizontal"
        if row[8]:
            ET.SubElement(game, "type").text = row[8]
        if row[9]:
            ET.SubElement(game, "joyways").text = row[9]
        if row[10]:
            ET.SubElement(game, "buttons").text = row[10]
